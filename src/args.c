  ...
  for (uint64_t i = 0; i < seeds->count; i += 1)
    {
      switch (seeds->items[i].type)
        ...
        case seed_item_tuple:
          {
            total += (seeds->items[i].content.to - seeds->items[i].content.from
                      + 1);
            break;
          }
        }
...
seeds_list_push_item (seeds_list *restrict list, seed_item item)
{
  ...
  // Already exists, but need to reallocate
  else if (list->size <= list->count)
    {
      ...
      list = realloc (list, sizeof (seeds_list)
                                + new_list_size * sizeof (seed_item));
      if (NULL == list)
        {
...
bool
try_next_seed (seeds_list *restrict *const seeds,
               uint64_t *restrict return_seed)
{
...
{
  fprintf (stdout,
           "usage: %s [-h] [-o folder/] [-l solver] [-p count] [-s seeds...] "
           "<path/to/file.data>\n",
           exec);
  puts ("  -h          print this help message.");
  puts ("  -o          output folder to save solutions. adds the seed to the "
        "data file name.");
  ...
  while ((opt = getopt (argc, argv, "hs:o:l:p:")) != -1)
    {
      switch (opt)
            ...
            if ('\0' == *end)
              {
                seeds = seeds_list_push_item (
                    seeds, (seed_item){ .type = seed_item_single,
                                        .content = { .single = from } });
              }
            else if ('-' == *end)
              {
                ...
                if (*end != '\0')
                  {
                    fprintf (stderr,
                             "failed to get the upper value of the range\n");
                    exit (EXIT_FAILURE);
                  }
                else if (from >= to)
                  {
                ...
                seeds = seeds_list_push_item (
                    seeds,
                    (seed_item){ .type = seed_item_tuple,
        ...
        case 'l':
          {
            for (uint64_t i = 0; i < sizeof (solvers) / sizeof (wfc_solver);
                 i += 1)
              {
        ...
        case 'p':
          {
            if ((parallel = to_u32 (optarg, &end)) <= 0)
              {
                fputs ("you must at least process one seed at a time...",
                       stderr);
                exit (EXIT_FAILURE);
              }
            else if ('\0' != *end)
